...
var timer = false;
function reloadingfunction(activetab,intervel_time,startDate,endDate) {
...
function call_main_func_main(minutes, activetab, intervel_time, startDate, endDate) {
	...
	if (!timer) {
		...
		if (!activetab.url.includes("chrome://")) {
			...
			waitTimer  = setInterval(reloadingfunction,time_to_wait_in_seconds,activetab,intervel_time,startDate,endDate);
			var theValue='jaes';
